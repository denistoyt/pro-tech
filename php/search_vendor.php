<?
    ...
    $search = "SELECT * FROM `items` WHERE `item_vendor` LIKE ?";
    $search_d = $pdo->prepare($search);
    $search_d->execute(array("%$_GET[item_vendor]%"));
    $search_ex = $search_d->fetchAll();
    foreach($search_ex as $arr) {
        $type = "";
