<?
    ...
    $cart_data = "SELECT items.id_item, items.item_name, items.item_img, items.item_img_alt, items.item_cost, cart.id_cart_item FROM `cart` INNER JOIN `items` ON cart.item_id = items.id_item WHERE cart.user_id = ?";
    ...
    foreach($cart_data_ex as $cart) {
        echo '
            <div class="cart__item" data-cartitemid="'.$cart['id_cart_item'].'" data-itemid="'.$cart['id_item'].'">
